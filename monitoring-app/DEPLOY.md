...
| `GW_PER_KEY_CONCURRENCY` | `4` | сколько параллельных запросов (агентов) может держать один API-ключ |
| `GW_GLOBAL_CONCURRENCY` | `16` | сколько параллельных запросов по всем ключам сразу |
| `GW_QUEUE_TIMEOUT` | `30` | сколько секунд запрос ждёт в очереди шлюза, потом 503 |
